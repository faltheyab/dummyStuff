...
spark.conf.set("spark.hadoop.fs.automatic.close", True)                     # Retry limit
spark.conf.set("spark.hadoop.fs.stocator.threads.core", 16)                     # Retry limit
spark.conf.set("spark.hadoop.fs.stocator.threads.max", 64)                     # Retry limit
spark.conf.set("spark.hadoop.fs.s3a.connection.maximum", 100)                     # Retry limit
...
while True:
    ...
    if (random_num % 2 == 0):
        read_write_app("ID_DATA_Example.csv","output_even_example6.csv")
    else:
        read_write_app("Employees.csv","output_odd_example6.csv")
